...
{
    public partial class PC09_sub : Form
    {
        public PC09_sub()
        {
            InitializeComponent();
       ...
       // BLL.FMD020 _bllFMD020 = new BLL.FMD020();
        Model.fmd040 _modelFMD040 = new Model.fmd040();
        BLL.fmd000 _bllFmd000 =new BLL.fmd000();
        BLL.fmd040 _bllFMD040 = new BLL.fmd040();
        public string _wholePath = null;                 //最终路径
        public string _conYCLBH = string.Empty;
        ...
        private void Window_Loaded(object sender, EventArgs e)
        {
            ...
            if (!"".Equals(_conYCLBH)) //如果社员编号有值，加载社员信息，否则清空控件
            {
                //txtYCLBH_GHS.Text = _conPM;
                txtYCLBH_BS.Text = _conYCLBH;
                //txtYXQ.Text = _conCLBH;


               // txtYCLBH_GHS.ReadOnly =
                    txtYCLBH_BS.ReadOnly=
                 //   txtYXQ.ReadOnly=
        ...
        private void LoadFormData()  //画面数据加载
        {
            try
            {
               // _modelFMD040.YCLLX = _conYCLBH;
                _modelFMD040.YCLBHBS = _conYCLBH;

                //_modelFMD040.ZT = "";

                DataTable ds = _bllFMD040.GetDaTaForPC08Spread("", _modelFMD040.YCLBHBS, "","=");     
                if (ds.Rows.Count > 0)
                {
        ...
        private void LoadControlData(DataTable ds)  //画面控件数据加载
        {
            try
            {
                ...
                txtYCLBH_GHS.Text = ds.Rows[0]["YCLBHGHS"].ToString();                          
                txtYCLBH_BS.Text = ds.Rows[0]["YCLBHBS"].ToString();    
        ...
        private void ClearEmptyExceptSYBH()  //除社员编号以外的画面控件清空
        {
            #region 控件赋值
            //基本信息
            txtYCLBH_BS.Text = string.Empty;     //品号

            txtYCLBH_GHS.Text = string.Empty;//品名
           // cboCLBH.Text =//材料类别
            cboYCLLX.Text =//计量单位
                // cboFLH.Text =//分类号
            txtZXKC.Text =//最小库存
        ...
        {
            txtYCLBH_GHS.Text = string.Empty;
            ClearEmptyExceptSYBH(); //除社员编号以外的画面控件清空
        }
        #endregion
        #region 验证事件
        ...
        {//验证顺序：画面控件从左到右，从上到下依次验证
            bool IsCheck = true;
            #region 验证
            //if (txtYCLBH_GHS.Text.Trim().IsNullOrEmpty())
            //{//品名
            ...
            if (txtYCLBH_BS.Text.Trim().IsNullOrEmpty())
            {//品号
                ComForm.DspMsg("W002","原材料(本社)");   //项目不能为空！
                txtYCLBH_BS.Focus();
                IsCheck = false;
                return IsCheck;
            }
        ...
        private void btnSaveOK()    //保存处理
        {
            ...
            YCLLX = StaticFun.getValue(cboYCLLX);    
            ...
            YCLBHBS = txtYCLBH_BS.Text.Trim();
            YCLBHGHS = txtYCLBH_GHS.Text.Trim();
            ...
            _modelFMD040.YCLBHGHS = StaticFun.strReaplace(YCLBHGHS); 
            _modelFMD040.YCLLX = YCLLX;
            _modelFMD040.ZT = "0";
        ...
        public void SaveFMD310(fmd040 model)  //保存到数据库
        {
            try
            {
                ArrayList al = new ArrayList();
                if (_bllFMD040.Exists(model.YCLBHBS))  //判断社员编号是否存在
                {
