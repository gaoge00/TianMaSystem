...
{
    public partial class PC14 : Form
    {
        ...
        static class SpdColNo
        {
            ...
            public static int QHZKSL = I++;           
            public static int PDR = I++;
        ...
        struct SpdDataSoureColName
        {
            ...
            public const string QHZKSL = "QHZKSL";
        ...
        private void SetSpdFormat()
        {
            try
            {
                ...
                fspd.ActiveSheet.Columns[SpdColNo.QHZKSL].HorizontalAlignment = CellHorizontalAlignment.Right;
                ...
                fspd.ActiveSheet.Columns[SpdColNo.PDRQ, SpdColNo.PDR ].CellType = textCellType;
               // fspd.ActiveSheet.Columns[SpdColNo.PDRQ, SpdColNo.PDR ].Width = 108F;




                fspd.ActiveSheet.Columns.Get(SpdColNo.YCLRKLOTNO).Width = 140F;
                fspd.ActiveSheet.Columns.Get(SpdColNo.PDRQ).Width = 135F;
                fspd.ActiveSheet.Columns.Get(SpdColNo.YCLBH).Width = 130F; 
                fspd.ActiveSheet.Columns.Get(SpdColNo.YCLLX).Width = 110F; 
                fspd.ActiveSheet.Columns.Get(SpdColNo.DW).Width = 108F; 
                fspd.ActiveSheet.Columns.Get(SpdColNo.SPSL).Width = 120F;
                fspd.ActiveSheet.Columns.Get(SpdColNo.QHZKSL).Width = 112F;
                fspd.ActiveSheet.Columns.Get(SpdColNo.PDR).Width = 102F; 
               ...
               // fspd.ActiveSheet.Columns.Get(SpdColNo.BZ).Locked = false;
                ...
                fspd.ActiveSheet.ColumnHeader.Columns.Get(SpdColNo.PDRQ).Label = "盘库日期";// 
               ...
               // fspd.ActiveSheet.ColumnHeader.Columns.Get(SpdColNo.YCLBHGHS).Label = "原材料(供货商)";// 
                ...
                fspd.ActiveSheet.ColumnHeader.Columns.Get(SpdColNo.SPSL).Label = "实盘数量";// 
                fspd.ActiveSheet.ColumnHeader.Columns.Get(SpdColNo.QHZKSL).Label = "前回在库数量";// 
                fspd.ActiveSheet.ColumnHeader.Columns.Get(SpdColNo.PDR).Label = "盘点人";// 
        ...
        private void Spd_Form_Load() //绑定Spread的数据
        {
            try
            {
                ...
                string strStartRq = dateStartRq.Value.ToString("yyyy/MM/dd", System.Globalization.DateTimeFormatInfo.InvariantInfo);   //开始日
                 ...
                 dtspd = _bllfed030.GetDaTaForPC08Spread(strStartRq, strEndRq, StaticFun.getValue(cboYCLLX));
                ...
                for (int i = 0; i < dtspd.Rows.Count; i++)
                {
                    ...
                    double.TryParse(dtspd.Rows[i][SpdDataSoureColName.QHZKSL].ToString(), out KCS);
                    fspd.ActiveSheet.Cells[i, SpdColNo.QHZKSL].Text = KCS.ToString("N4");

                    
                    fspd.ActiveSheet.Cells[i, SpdColNo.PDR].Text = dtspd.Rows[i][SpdDataSoureColName.PDR].ToString();
        ...
        string Save_Path = System.Configuration.ConfigurationSettings.AppSettings["Path_PC14"].ToString();
        public void SetExcel()
        {
            try
            {
                ...
                dt = _bllfed030.GetDaTaForPC08Spread(strStartRq, strEndRq, StaticFun.getValue(cboYCLLX));
                if (dt.Rows.Count > 0)
                {
                    if (ComForm.DspMsg("Q012", "").Equals(ComConst.LING))
                    {
                        ...
                        Ebe.OpenEFile(System.Windows.Forms.Application.StartupPath + "\\Model\\PC14.xls");


                        for (int i = 0; i < dt.Rows.Count; i++)
                        {

                            Ebe.SetCellValue(3 + i, 1, dt.Rows[i][SpdDataSoureColName.PDRQ].ToString());
                            Ebe.SetCellValue(3 + i, 2, dt.Rows[i][SpdDataSoureColName.YCLBH].ToString());
                            Ebe.SetCellValue(3 + i, 3, dt.Rows[i][SpdDataSoureColName.YCLLX].ToString());
                            Ebe.SetCellValue(3 + i, 4, dt.Rows[i][SpdDataSoureColName.YCLRKLOTNO].ToString());

                            Ebe.SetCellValue(3 + i, 5, dt.Rows[i][SpdDataSoureColName.DW].ToString());
                            Ebe.SetCellValue(3 + i, 6, dt.Rows[i][SpdDataSoureColName.SPSL].ToString());
                            Ebe.SetCellValue(3 + i, 7, dt.Rows[i][SpdDataSoureColName.QHZKSL].ToString());
                            Ebe.SetCellValue(3 + i, 8, dt.Rows[i][SpdDataSoureColName.PDR].ToString());
