...
{
    public partial class WinSubKey_Value : Form
    {
        public WinSubKey_Value()
        {
            InitializeComponent();
        }

        public WinSubKey_Value(string BMBH)
        {
            this._conBMBH = BMBH;
            InitializeComponent();
        }
        public WinSubKey_Value(string winQufen, string BM)
        {
        ...
        public string _conKEY = String.Empty;
        ...
        public string _conValue = String.Empty;
        ...
        private void Window_Loaded(object sender, EventArgs e)  //DataGrid数据绑定
        {
            ...
            _modelFMD310.SYBH = _conKEY;

            ds = _bllFMD310.GetDaTaForWinSubKey_Value("FMD030","","" );//(_modelFMD310, _conValue);
            FillSpread(ds);
        }
        ...
        private void FillSpread(DataSet ds)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    fpsSYDate.ActiveSheet.Cells[i, 0].Text = ds.Tables[0].Rows[i]["strKEY"].ToString();
                    fpsSYDate.ActiveSheet.Cells[i, 1].Text = ds.Tables[0].Rows[i]["strValue"].ToString();                  
        ...
        private void fpsSYDate_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)//双击事件
        {
            ...
            _conKEY = fpsSYDate.ActiveSheet.Cells[ActRow, 0].Text.ToString();
           
            _conValue = fpsSYDate.ActiveSheet.Cells[ActRow,1].Text.ToString();
